# Auth Strategies
*Authentication strategies implemented from scratch in TypeScript, one branch per strategy*
...
## Decisions
The implementation will use DB with document orientation, because we need flexibility between the branches (features) and the relation with other models are not strong. For the JWT-based implementation: keep the user logged in (Refresh Token) and recovery the user's passowrd.
